...
def make_list():
    global header
    line_counter = 0
    w_name = input("KNN을 적용할 사이트를 입력하세요(_data.csv 생략): ")
    with open(w_name + "_data.csv", 'rt', encoding='UTF8') as f:
        while 1:
...
# 그래프 출력
def draw_knngraph(data_list, state_list, target):
...
def k_test(target_list):
    for k_num in range(1, 11):
        ...
        for seq in target_list:
            target = [seq[0], seq[1]]
            k = k_num
            class_result = classify(dataset, target, class_category, k)
            dic = {3: "일시적 유행", 2: "확산", 1: "일반", 0: "정체"}
            if dic[seq[2]] == classify_result(class_result): #예상 상태와 같은 값이 나오면 match+
                match += 1
...
make_list()
# dataset 생성
dataset, class_target, class_category = data_set(cp_list, state_list)
#print(state_list)
...
print(a + "상태입니다.")
draw_knngraph(cp_list, state_list, target)
